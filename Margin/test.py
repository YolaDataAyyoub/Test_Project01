...
for section, details in dict_sections.items() : 
    if details['column_name'] : 

        rangeClear = day_letter + str(details['range_start_index']) + ":" + day_letter + str(details['range_end_index'])
        rangeUpdate = day_letter + str(details['range_start_index'])

        df_material_journey_section = fromGoogleSheets(SAMPLE_SPREADSHEET_ID_MaterialJourney, SHEET_NAME=sheet_name_MaterialJourney + '!' + item_range_letter + str(details['range_start_index'] - 1) + ':' + item_range_letter + str(details['range_end_index']))
        df_material_journey_section = df_material_journey_section.rename(columns={section : 'Material ID'})
        df_result_merged = df_material_journey_section.merge(data, on='Material ID', how='left')

        df_result_merged.fillna(0, inplace=True)

        toGoogleSheets(df_result_merged[[details['column_name']]], SAMPLE_SPREADSHEET_ID_MaterialJourney, sheet_name_MaterialJourney, rangeClear, rangeUpdate, SCOPES=['https://www.googleapis.com/auth/spreadsheets'])
        print('='*30)
