#Good, Done for pointage app
import pandas as pd
from yola_functions import toGoogleSheets, fromGoogleSheets,addBlanckRow
import sys
...
print(Df_pointage)
Df_pointage['DT'] = pd.to_datetime(Df_pointage['Date de pointage'] + ' ' + Df_pointage['L\'heure'], format='%d/%m/%Y %H:%M:%S')
...
for employe in employes:
    for date in dates_pointage:
        ...
        # Group the DataFrame by employee and date, then extract the last 'heure de pointage' for each group
        ...
        result = pd.concat([result, pd.DataFrame({'Nom d\'employé': [employe],
                                                   ...
                                                   'l\'heure de pointage': [last_heure_pointage]})], ignore_index=True)
